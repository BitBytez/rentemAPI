...
router.get('/buy', async(req,res)=>{
    var responseData = {};
    const ads = await client.db(ads_db_name).collection('ads').find({},{projection:{_id:0}}).toArray();
    if (ads){
    ...
    const trending = await client.db(trending_db).collection('trend_buy').find({}, {
        projection: {
            _id: 0
        }
    }).toArray();
